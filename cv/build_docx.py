#!/usr/bin/env python3
"""Generate cv-2026-06-19-ops-rq11154-senior-dev.docx from structured content.
...
EXPERIENCE = [
    {
        "org": "Abrams Software", "loc": "Kingston / Toronto, Ontario (Remote)",
...
name_run = run(NAME, size=SZ_NAME, bold=True)
loc_run = run(LOCATION, size=SZ_LOC, color=MUTED)
body.append(two_col(name_run, loc_run, before=0, after=20))

# Tagline
body.append(para(run(TAGLINE, size=SZ_TAG, color=MUTED),
...
body.append(body_para(SUMMARY, before=20, after=120))
...
body.append(body_para(CERTS, before=20, after=100))
...
out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        "cv-2026-06-19-ops-rq11154-senior-dev.docx")

...
#show: cv.with(
  ...
  tagline: "Senior Full-Stack .NET Developer · C\\# / ASP.NET / .NET Core / Web API / Blazor WASM / WCF · Entity Framework / LINQ · Azure (Functions, App Service, Entra ID, Key Vault, Managed Identities, DevOps CI/CD) · REST / Swagger / Postman / OAuth / CORS · SQL Server · JavaScript / TypeScript · Crystal Reports · xUnit / Moq · WCAG / AODA · Agile / Scrum · 15+ Years",
  contact: (
    "(819) 921-7192",
  ...
  summary: [
    *Senior full-stack .NET developer with 15+ years across the full SDLC*, shipping *enterprise web applications* from front-end UI through service back ends and relational databases — the bulk of it in the *Ontario- and federal-scale public sector*. Deep *C\# / .NET* engineering: *ASP.NET / ASP.NET MVC, .NET Core, Web API, Entity Framework, LINQ, ADO.NET*, and a *Blazor* front end, with *WCF* and *WPF* on legacy tiers. Designs and consumes *RESTful APIs between services* documented and tested with *Swagger, Postman, and ThunderClient*, secured with *OAuth 2.0 / OpenID Connect, client- and server-side CORS policy, and API-management throttling*. Strong on *Azure application development and security* — *Azure Functions, App Service, Container Instances, Entra ID / Azure AD, MFA / SSO, App registrations, Key Vault, and Managed Identities*. Front end in *JavaScript, HTML / CSS, and React + TypeScript* (component-based SPA patterns; framework-transferable to Angular), built to *full WCAG AA / AODA accessibility compliance*. Designs and models *relational databases* and writes performant *T-SQL* against *SQL Server*. Disciplined engineering practice: *SOLID design, dependency injection, reusable NuGet components, unit testing with xUnit / Moq*, code review, and *Azure DevOps CI/CD pipelines*. Builds operational and management *reports* (SSRS, Crystal Reports, Power BI). Delivers in *Agile / Scrum* (Professional Scrum Master I) tracking work in *Azure DevOps / TFS and JIRA*. Active open-source contributor — *60+ public PRs* including Microsoft's `vsts-team-calendar`, `tfs-cli`, and the *Microsoft Authentication Library .NET* docs.
  ],
  experience: (
    (
      org: "Abrams Software",
...
*.NET / Back-End:* *.NET Core / .NET 6 / 8, ASP.NET / ASP.NET MVC, Web API, ADO.NET, Entity Framework, LINQ* · *Blazor (WASM)* · *WCF, WPF* (legacy) · *SOLID, dependency injection, reusable NuGet components* · *Node.js, Express, NestJS* · Spring Boot (familiarity)

*Front-End / UI:* *JavaScript, HTML / CSS, React 19, TypeScript*, Blazor, Knockout.js MVVM, Vite, AJAX, jQuery · component-based SPA (Angular-transferable) · *responsive, cross-browser, full WCAG AA / AODA accessibility* · UX/UI fundamentals

*APIs:* *RESTful Web API design between services*, GraphQL, WebSocket · *Swagger, Postman, ThunderClient* · *OAuth 2.0 / OpenID Connect, CORS* · API gateway / management (Azure APIM) · SOAP / WCF (legacy)

*Azure:* *Azure Functions, App Service, Container Instances, Azure SQL, API Management, Key Vault, Managed Identities* · *Entra ID / Azure AD — OAuth 2.0 / OpenID Connect, MFA / SSO, App registrations & policies, CORS*
...
*CI/CD & DevOps:* *Azure DevOps Pipelines (YAML), Git, GitHub Actions* · *Docker, Kubernetes* · Azure Data Factory (ETL / ELT) · feature branches, PR-gated CI
...
*Adjacent / rapidly transferable:* Deep relational-database and *T-SQL* experience transfers directly to *Oracle / PL-SQL* (and TOAD as a client); *component-based SPA* fluency across *React, Blazor, and Knockout.js MVVM* transfers directly to *Angular*; comfortable adding *Redis* distributed caching to the .NET / Azure stack.
